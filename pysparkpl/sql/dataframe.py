...
class DataFrame:
    ...
    def join(
        ...
        if how == "right":
            ...
            joined_l = self._df.join(other._df, on, how="left", suffix=suffix)
            joined_r = other._df.join(self._df, on, how="left", suffix=suffix)
            joined = None
        else:
            h = how
...
def _breakdown_on_expr(e: col, dfl, dfr):
    ...
    if e._op[-1] == "__eq__":
        if _dfs_in_expr(e._op[0]) == set([dfl]) and _dfs_in_expr(
            e._op[1]
        ) == set([dfr]):
            return [e._op[0]], [e._op[1]], None, None
        if _dfs_in_expr(e._op[0]) == set([dfr]) and _dfs_in_expr(
            e._op[1]
        ) == set([dfl]):
            return [e._op[1]], [e._op[0]], None, None
